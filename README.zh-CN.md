...
普通 HTML 可以很快出第一版。但当 slides 变大、需要精调某一页，或者反复修改 CSS、动画
和布局时，一次改动很容易悄悄影响其他页面。这个 Skill 主要解决的就是更严谨阶段的问题。

它的核心优势不只是做出更漂亮的 slides，而是把 deck 当成一个可测试的 Web 应用：scene
...
这个 Skill 适合设计质量、多轮迭代和交付检查都比较重要的 slides。
