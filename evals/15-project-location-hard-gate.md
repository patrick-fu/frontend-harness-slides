...
description: >-
  Before scaffolding a new slide project with no reliable specified target, the
  agent must confirm the exact deck root instead of assuming the current
  directory.
difficulty: medium
...
- [ ] Does not run scaffold commands or create files before the `deck root` is known.
- [ ] Treats this as a long-term deliverable with no reliable specified target, so it asks the user to confirm the exact `deck root` / project directory before file creation.
